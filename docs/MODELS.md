...
| Benchmark-Tab mit versionierten Suiten | ✅ 2026-09-17 `core::bench::suites` (`chat-v1` 256 Tok., `coding-v1` 384 Tok., Migr. `0017`: `suite` + `detail_json`) — feste Ausgabelänge über `GenerationOptions::fixed_length()` (`ignore_eos`, `temperature 0`, `seed 0`, `cache_prompt false`), Pro-Prompt-Detail, Stabilität als Mittel der Pro-Prompt-Stabilitäten. **HTTP**: `GET /bench/suites`, `POST /models/{id}/benchmark` mit optionalem Body `{suite, runs}` (leerer Body = alter Quick-Test), `GET /benchmarks/history?suite=&limit=` (Limit `1..=200`). UI: `ui/src/features/benchmark/`. Coding-**Korrektheit** (Sandbox) + andere Runtimes = später, siehe `docs/TODO.md` |
