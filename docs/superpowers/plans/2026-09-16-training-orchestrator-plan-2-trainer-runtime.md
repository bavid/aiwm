...
- [ ] **Step 1:** download the 4B base once with the `hf` CLI into `<store>/training/flux2-klein-4b/` (exclude `*.jpg` and the single-file duplicate; ≈ 15.7 GB) **[CORRECTED 2026-09-17 by the real run: this single file is the one ai-toolkit's klein loader reads; the diffusers `transformer/` layout is unused — see docs/TODO.md Teilsystem 2]**, compute per-file SHA-256 locally, cross-check against the HF API values in the appendix, register as a directory model with role `training_base_flux2_klein_4b`; pin the computed hashes in `TRAINING_BASES` and add a `verify_base_dir(dir, base) -> Result<()>` used by the preflight (size + hash spot-check of the largest file only, full check on demand).
