...
///
/// The `Value`-level version, for the recipes below that still build their
/// graphs as inline `json!`. The image recipes use
/// [`fragments::loras::apply`], the same chain against a
/// [`graph::Graph`]; this one goes away once the remaining recipes are ported
/// too.
fn apply_loras(
    g: &mut Value,
    loras: &[LoraSpec],
